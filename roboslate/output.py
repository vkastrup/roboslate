"""
Result formatting and output for RoboSlate.
...
from roboslate import __version__
...
# Standard camera reel pattern: a capital letter followed by 3 digits at the
# start of the filename — e.g. "A051" in "A051C002_230614_A4LG.mp4"
...
def disambiguate_roll(merged_fields, filename):
    ...
    Modifies merged_fields in place. Adds 'filename_reel' to the roll field
    to record where the value came from.

    E.g.: roll = "A051, B042", filename reel = "A051" → roll = "A051" (high confidence)
    """
    ...
    if "," not in value:
        # Single value — confirm it against the filename if it matches
        if value.upper() == reel:
            roll_field["confidence"] = "high"
            roll_field["filename_confirmed"] = True
        return

    # Multiple values — pick the one matching the filename reel
    ...
    if match:
        ...
        roll_field["filename_confirmed"] = True
    # If no match, leave all candidates and let it stay as a conflict
...
def classify_status(merged):
    ...
    # Slate detected — check if any fields were extracted
...
def build_result(video_path, merged, scan_metadata, shot_info=None):
    ...
    Args:
        ...
        scan_metadata: Dict with: duration_seconds, frames_scanned,
                       api_calls_made, phases_run.
        shot_info: Optional dict with SCRATCH shot metadata (uuid, name, project, etc.).
    ...
    # Disambiguate multi-camera roll values using the filename reel code
    ...
    result = {
        ...
        "result": {
            ...
            "api_calls_made": scan_metadata.get("api_calls_made", 0),
...
def write_csv_row(result, csv_path, fields=None):
    ...
    Per-field confidence is encoded as field_name_confidence columns.
    fields: list of field names to include; None → all SLATE_FIELDS.
    """
    ...
    row.update({
        ...
        "api_calls_made":     res.get("api_calls_made"),
        "processed_at":       result.get("processed_at"),
        "source_file":        src.get("file"),
        "scratch_shot_uuid":  src.get("scratch_shot_uuid"),
    })
...
def print_summary(result, fields=None):
    ...
    fields: list of field names to show; None → all fields present in result.
    """
    src = result.get("source", {})
    res = result.get("result", {})
    slate = result.get("slate") or {}
    ...
    print(f"  Frames scanned : {res.get('frames_scanned')} ({res.get('api_calls_made')} API calls)")
