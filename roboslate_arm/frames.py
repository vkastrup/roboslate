...
def get_video_duration(video_path):
    ...
    if result.returncode != 0:
        raise RuntimeError(f"ffprobe failed on {video_path!r}:\n{result.stderr.strip()}")
...
def extract_phase_range(video_path, start_time, duration_secs, fps, output_dir, prefix, jpeg_quality=3):
    ...
    cmd = [
        ...
        "-r", str(fps),
        "-vframes", str(int(duration_secs * fps) + 2),
...
def extract_single_frame(video_path, timestamp, output_dir, jpeg_quality=3):
    ...
    if not os.path.isfile(output_path):
        raise RuntimeError(f"ffmpeg produced no output at {timestamp}s for: {video_path}")
...
def make_temp_dir(base_dir=None):
    """Create and return a temporary directory for frame storage."""
    if base_dir:
        os.makedirs(base_dir, exist_ok=True)
        return tempfile.mkdtemp(prefix="frames_", dir=base_dir)
    return tempfile.mkdtemp(prefix="roboslate_arm_")
