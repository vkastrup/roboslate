"""
Configuration loader for RoboSlate-arm.

Reads a key=value config file (config.env). No third-party dependencies.

Search order:
  1. Explicit path passed to load_config()
  2. config.env in the same directory as roboslate-arm.py
  3. ~/.config/roboslate-arm/config.env
"""
...
def load_config(explicit_path=None, project_dir=None):
    ...
    Args:
        ...
        project_dir: Directory of roboslate-arm.py (used to find sibling config.env).
    """
    search_paths = []
    ...
    search_paths.append(os.path.expanduser("~/.config/roboslate-arm/config.env"))
